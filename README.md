...
```bash
pip install webcolors pillow numpy scikit-learn
```

可选工具：

- `agent-browser`：用于 `--analyze-url` 网页采样
- `@google/design.md` CLI：用于 `--export` 导出（不提供时会使用内置 fallback）
...
# 分析网页
python scripts/generate_design_spec.py --analyze-url https://example.com
...
design-spec/
├── scripts/
│   ├── generate_design_spec.py   # 主入口：生成、导出、自检
│   ├── analyze_screenshot.py     # 截图分析
│   ├── analyze_url.py            # 网页采样
│   └── extraction.js             # 网页样式提取脚本
├── references/
│   └── DESIGN.md                 # 参考示例
...
- 截图分析和网页采样都是启发式推断，复杂页面建议人工复核后再投入生产
- `--analyze-url` 需要提前安装 `agent-browser`
